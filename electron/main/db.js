...
function normalizeFrameworkProvenance(value = {}) {
  const provenance = value && typeof value === 'object' && !Array.isArray(value) ? value : {};
  const createdAt = String(provenance.created_at || new Date().toISOString()).trim();
  return {
    ...provenance,
    ...
    imported_at: String(provenance.imported_at || createdAt).trim(),
    imported_at_central: String(provenance.imported_at_central || formatCentralTime(new Date(createdAt))).trim()
  };
}
